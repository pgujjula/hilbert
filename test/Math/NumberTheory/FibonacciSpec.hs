...
import Math.NumberTheory.Fibonacci (fibonacciN, fibonaccis, lucasNumN,
                                    lucasNums, lucasSeq, lucasSeqN)
import System.Random               (Random)
import System.Random.Stateful      (Uniform, UniformRange, uniformM, uniformRM,
                                    uniformWord32R)
...
lucasSeqSpec = it "correct for arbitrary seeds" $
    let test :: (Num a, Random a, Show a, Ord a)
             => Proxy (Gen (a, a))
             -> Property
        test proxy =
